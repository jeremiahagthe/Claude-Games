...
//
// STALE-DIST RULE: this executes packages/tank-client/bin/tankwait.js, which
// loads dist/. Build first: `npm run build -w tankwait-core && npm run build -w
// tankwait`. A stale dist tests stale code. The offline countdown/anim repaint
// means frames accumulate with zero input.
  ...
  it('headless offline run produces homed, in-bounds 80x24 frames from the REAL binary', async () => {
    const out = await new Promise<string>((resolve, reject) => {
      const child = execFile(
        'node',
        ['packages/tank-client/bin/tankwait.js', '--offline', '--seed', '1'],
