import type { Difficulty } from 'snakewait-core'

const DIFFICULTIES: readonly Difficulty[] = ['easy', 'normal', 'hard']

// snakewait routes live on the same Cloudflare Worker host as
// fragwait/checkwait/boomwait's — one deployment serves all four games'
// Durable Objects (the real online routes land in the next task; this
// default just points there ahead of time).
...
export function parseArgs(argv: string[]): CliOpts {
  // Default difficulty is 'easy', mirroring boomwait's cliArgs: feel-gating
  // found normal's bots near-hard in practice. Harder bots are an explicit
  // opt-in via --difficulty.
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--offline') opts.offline = true
      ...
      throw new Error(`unknown flag: ${a} (usage: snakewait [--offline] [--name x] [--server url] [--seed n] [--difficulty easy|normal|hard])`)
    }
  }
  return opts
}
