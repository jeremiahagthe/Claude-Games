...
// (this task) and online.ts (next task) each drive their own tick loop BODY
// (synchronous bot decisions vs. server relay) but share everything here —
// mirrors packages/bomber-client/src/game.ts's split, right down to
// teardownAndExit's contract (finale screen → waitForPress → close listener
// → restore → share card on the NORMAL screen). Transcribed from bomber's
// game.ts with the bomb flag removed (Input here is just { dir }) and
// layout() widened to Layout | null: snake's minimum fit is 80x24 with no
// smaller fallback, so chooseLayout can return null below that (bomber's
// chooseLayout never does).
import type { Input, Result } from 'snakewait-core'
...
// client I/O shell, same as fragwait/boomwait's loops).
...
export interface GameSession {
  ...
  // null below snakewait's 80x24 minimum fit — caller renders tooSmallScreen
  // instead of a frame and keeps polling for a resize back above it.
  layout(): Layout | null
  // Pulls this tick's Input off the latch (one-shot dir pulse).
  drainInput(): Input
...
export async function setupGame(): Promise<GameSession> {
  ...
  const onData = (chunk: Buffer): void => {
    for (const e of parser.feed(chunk)) {
      if ('type' in e) continue // snake has no mouse input
  ...
  const onResize = (): void => {
    ...
    // the layout's k/HUD shape changed (chess-4 lesson).
    term.write('\x1b[2J')
    resizeCb?.()
  }
