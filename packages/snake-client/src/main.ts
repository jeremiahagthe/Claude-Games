...
// offline is explicitly wanted, e.g. no server reachable). Any join/connect failure while going
// online (join HTTP error, timeout, ws error, or a pre-start ws close) falls back to the same
// offline loop exactly once — runOnline only ever returns 'fallback' from a pre-match failure;
// once a match has actually started it always finishes and exits the process itself
...
export async function main(opts: CliOpts): Promise<void> {
  ...
  if (!opts.offline) {
    ...
    console.log('snakewait: could not reach an online match — playing offline\n')
  }
