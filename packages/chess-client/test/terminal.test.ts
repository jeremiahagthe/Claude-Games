...
// Mirrors packages/client/test/terminal.test.ts, minus the OSC 22 crosshair
// assertions — this copy drops the crosshair pointer-shape request since
// chess wants the normal OS pointer.
...
describe('TerminalSession', () => {
  ...
  it('enter enables the whole any-motion mouse ladder; restore disables it in exact mirror order', () => {
    ...
    // no OSC 22 crosshair request — chess keeps the normal OS pointer
    expect(all).not.toContain('\x1b]22;')
    written.length = 0
    t.restore()
    const rest = written.join('')
    expect(rest).toContain('\x1b[?1004l')
    expect(rest.indexOf('\x1b[?1004l')).toBeLessThan(rest.indexOf('\x1b[?1006l'))
    expect(rest).not.toContain('\x1b]22;')
