...
// normal: power ±6%, angle ±1°, correction gain 0.4  [AMENDED at Task 4: original ±12%/±3°/0.7 fails the gates with the 0.70 short opener; retune was authorized, gates unchanged]
...
- Start-deadline no-show guard [AMENDED at Task 6 — the block "no-show lesson", omitted in the original plan]: when the first socket connects to a `humanCount=2` match, set a START_DEADLINE_MS=15_000 alarm; if the second human never connects by then, convert that slot to a 'normal' bot and start — preserving the family promise that a joined player always gets a game.
